...
#ifdef HAVE_LIBHANGUL
	search_text=g_strdup_printf("%s%s", search_input->text ? search_input->text : "", search_input->preedit ? search_input->preedit : "");
#else
...
#ifdef HAVE_LIBHANGUL
	char *search_text=0;
	search_text=g_strdup_printf("%s%s", wm->text ? wm->text : "", wm->preedit ? wm->preedit : "");
	if (search_text && g_utf8_strlen(search_text, -1) >= minlen) {
#else
